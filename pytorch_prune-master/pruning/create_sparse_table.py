...
def parameter_hist(temp_dict,curr_energy,max_spatial_ratio,prun_algo=1.0,prun_algo_tuning=0.5,bin_core_sparse_ratio_decision=os.path.join(os.path.dirname(__file__),'core_sparse_ratio_decision'),sparse_table={}):
    net_sparse_curr = 0
    net_sparse_max = 0
    MAX_SPARSE=0.99
    total_param=0.0
...
def create_sparse_table(model,input_shape,DEFAULT_SPARSE_STEP=[0.80, 0.82, 0.84, 0.86, 0.88, 0.90,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.99],sparse_table_path='sparse_table.json',init_energy=10):
    if os.path.isfile(sparse_table_path):
        ...
        while len(desire_index_table)<len(DEFAULT_SPARSE_STEP): # to find desired energy step
            ...
            glog.info("curr_energy, curr_sparse, prev_energy, prev_sparse, target_sparse, diff_energy, diff_prev_sparse, diff_target_sparse, energy_increment")
