...
def pruning(layers,sparse_table={},prefix=''):
    ...
    for i,key in enumerate(keys):
        ...
        if isinstance(layer_,torch.nn.Conv2d) or isinstance(layer_,torch.nn.Linear)or isinstance(layer_,torch.nn.ConvTranspose2d):
            sparse_coeff=sparse_table[layer_.layer_name] if layer_.layer_name in sparse_table else 0.0
            if isinstance(layer_,torch.nn.Linear):
                layer_.is_fc=True
            else:
                layer_.is_fc=False

            layer_.register_buffer('mask',torch.ones_like(layer_.weight))
            if sparse_coeff>0:
                weight_view=torch.abs(layer_.weight).view(-1)
                n_param=weight_view.size(0)
                point=int(np.round(sparse_coeff*n_param))
                sort_value,sort_index=weight_view.sort(descending=False)
                layer_.mask.view(-1)[sort_index[:point]]=0.0

                glog.info("{} {} PRUN THR: {} RATIO: {}".format(
                                                                layer_.layer_name,
                                                                layer_.__class__.__name__,
